...
**Embeddings** are just vectors that represent a piece of content—a song, a text, or [a piece of DNA]({% post_url 2025-05-02-hf-transformers %}# embedding-dna-sequences). You can think of embeddings as coordinates in some very high-dimensional space. They are usually the activations of a given internal layer of a pre-trained model. This ensures two things. First, that all the embeddings have the _same dimensions_, so we can neatly arrange them into a matrix. Second, that the representation is _meaningful_. After all, the model had to learn a good representation of the data during training. This also ensures that semantically similar items are near each other.
...
The cosine similarity is a common choice to measure semantic similarity of embeddings. Although I haven't found a satisfying explanation for the why, the lengths of the vectors do not carry interesting information over the direction in which they point.

For instance, here is the similarity matrix between all 29 blog posts:

{% include figure.liquid loading="eager" path="assets/python/2025-08-16-rags/img/posts_cosine_similarities_heatmap.webp" class="img-fluid" %}

## Nearest neighbor search

Let's assume I want to fetch the 5 blog posts best-matching this query text:

```
A blog post about interpretable machine learning.
```

We can also embed this text and compare that embedding against that of all my blog posts. In this case, here are the best 5 matches.

I was lucky I am not a prolific writer, we and I could compute all the distances. But this operation has a time complexity of $$O(N^2)$$, which makes it unfeasible for large collections. That's why I will focus on approximate methods, which produce good enough estimations of similarity in a reasonable time.
...
When an LLM produces an output, it uses two sources of information: its _memory_ and the _query_. The LLM's memory consists of large swaths of correlated texts stored in its weights. The query is the request posed by the user, containing a context and potentially some additional information. Vector databases provide a way to add a third source of information, allowing the LLM to efficiently find relevant items in a corpus and leverage them in its answer. This is called **retrieval augmented generation** or RAG.
