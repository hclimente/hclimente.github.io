...
Vector databases efficiently encapsulate the above to efficiently store vector embeddings and show similar items. I will show how to do the above using the [Qdrant vector database](https://qdrant.tech/). Efficient vector search has traditionally been very used in recommendation algorithms. However, the advent of LLM has infused them with renewed interest.
...
The idea is quite simple. First the user query is embeded and its closest neighbors in the corpus are retrieved as additional context. Then, both the query and the context are passed to the LLM, which will use both to generate the final answer.
