...
However, this brute-force approach has a time complexity of $$O(DMN)$$, where $$N$$ is the number of posts and $$M$$ the average number of chunks per post. For large collections, this becomes prohibitively slow. That's why we need approximate methods that can find good matches efficiently.
