...
{% include figure.liquid loading="eager" path="assets/python/2025-03-15-python-uv/mnist-classifier/img/mnist_examples.webp" class="img-fluid rounded z-depth-1" alt="Sample MNIST digits" %}

In my conventional workflow, I would have used `conda` to create an environment. I would install all the packages I need via a few, slow `conda install`s. Eventually I will start my work. If, during my model development I need to install additional packages, I will cross my fingers not to confront the dreaded `Solving environment: failed with initial frozen solve. Retrying with flexible solve.` error. Once I am done, I will dump my environment into an `environment.yaml` file, remove the dependencies that are only used in development, and hope that the final environment resembles enough the one I worked on. Then, I will build a Docker image based off this environment file to deploy the model.
