...
To establish this connection more formally, let’s start by recasting the Shapley value problem by mapping the original concepts to the machine learning context:
...
$$
f(\mathbf{x}) = g(\mathbf{x}') = \mathbb{E}[f(X)] + \sum_{j = 1}^m \phi_j
$$

since $$\mathbf{x}' = \mathbf{1}$$.
...
One key limitation of SHAP values is that they do not handle well **highly correlated features**. How models leverage them is highly arbitrary (e.g., picking only one feature per group or distributing the weights over all the feature). Consequently, the resulting SHAP values can produce a misleading representation or feature importance.

Another limitation of SHAP values as presented [above](#shap-values) is that they force the explanation function to take an additive form of the feature contributions which doesn't account explicitly for **feature interactions**. Note that the Shapley values do cover interactions, but the additive model $$g$$ doesn't account for their contribution separately. Nonetheless, SHAP values can be extended to include [feature interacitons](https://shap.readthedocs.io/en/latest/example_notebooks/tabular_examples/tree_based_models/Basic%20SHAP%20Interaction%20Value%20Example%20in%20XGBoost.html).

Last, it's worth pointing out that at no point do SHAP values consider the correctness of the model's prediction. Hence, if our model is wrong (e.g., overfit), we will still get a set of SHAP values that explain the prediction. Other measures of importance, like permutation feature importance in random forests, do take into account the performance of the model.

# Flavors of SHAP: the permutation approximation

[Above](#shap-values) I described the general approach to compute SHAP values. Unfortunately, it is very computationally intensive: exploring all possible coalitions is equivalent to exploring all $$2^m$$ subsets of features. For that reason, different flavors of SHAP values have been proposed to make computations more efficient. I describe below the **permutation method**, a model-agnostic method to compute SHAP values $$\phi_i$$. However, there are others specialized in specific model types, like [Tree SHAP](https://shap.readthedocs.io/en/latest/generated/shap.TreeExplainer.html) for tree-based models (e.g., random forests, gradient boosting) and [Deep SHAP](https://shap.readthedocs.io/en/latest/generated/shap.DeepExplainer.html) for deep learning models.

The permutation method approximates SHAP values by estimating the expected marginal contribution of each feature over many random permutations of the features. Let's study the permutation method through a worked out example. We aim to explain a prediction $$f(\mathbf{x}_0)$$. We have at our disposal a background dataset $$X_\text{bg}$$, e.g., the whole training set, which we will use for sampling. For the sake of the example, our model only considers four (simplified) features: $$\mathbf{x}_0 = [x_{\text{age}}, x_\text{sex}, x_\text{BMI}, x_\text{BP}]$$.
...
1. Approximate the SHAP value as the average marginal contribution: $$\phi_\text{age} \symeq \frac{1}{K} \sum_i \delta_{i}.$$
