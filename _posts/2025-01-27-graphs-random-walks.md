...
A **random walk (RW)** consists on a walker be located at a vertex and, at each timestep, move to one of the vertex's [adjacent](2025-01-23-graphs-glossary#adjacency) vertices. Often the transition probability between vertices is represented by the matrix $$A_{rw}$$, a normalized version of the [adjacency](2025-01-23-graphs-linear-algebra#adjacency-matrix) in which the weight of all outbound edges adds up to 1. The outcome of a single random walk is a [walk](2025-01-23-graphs-glossary#walk) of length $$t$$, where $$t$$ represents the number of timesteps. We can consider the outcome of infinitely many random walkers departing from a given vertex. In that case, the outcome at time $$t$$ is a vector $$n \times 1$$ $$\pi_t$$, in which $$\pi_{ti}$$ represents the probability of the walker starting at a given vertex and being on vertex $i$ at time $t$. The probability distribution at time $$t+1$$ is computed as:

$$
\pi_{t+1} = A_{rw} \pi_t
$$

In the case in which the graph has a single [component](2025-01-23-graphs-glossary#component), if we allow the RW to run indefinitely, the probability of being at any given vertex is uniform (TODO Why?):
