...
The main tool to ensure that our communications remain private is __encryption__. Encryption consists on reversibly transforming a message into an (apparently) random message using an encryption key. The decryption key, if you have it, allows you to recover the original information.

We will first focus on __symmetric__ encryption, i.e., the same key is used for both actions. Then, we'll move onto __asymmetric__ encryption, in which the keys are different.

# Symmetric Encryption: Advanced Encryption Standard

The [Caesar cipher](https://en.wikipedia.org/wiki/Caesar_cipher) is a primitive form of symmetric encryption. Our key is a number, which indicates how many letters we shift the alphabet by:
...
A key is secure when it cannot be guessed easily. This will happen when it is _long_ and can only be guessed via _brute force_. AES-128 has $$2^{128}$$ possible 128-bit keys, and testing them all is unfeasible. The algorithm is even stronger when testing each key is _expensive_.

{% enddetails %}

AES is pretty convoluted algorithm, and I find it a bit uninteresting. Kind of like an elaborate recipe to shuffle cards reproducibly; interesting and lucrative applications, boring to watch. Luckily for me there are many good, detailed explanations around the web I can point you to (e.g., [this one](https://www.geeksforgeeks.org/computer-networks/advanced-encryption-standard-aes/)).
...
But what about securing _communications_? How can Alice and Bob agree on an AES key in the presence of Eve, who will listen to every word they cross? In the old times, Alice and Bob would meet in a park and exchange keys in closed envelopes, making sure Eve can't get a peek. But in 1976 two researchers, Diffie and Hellman, introduced an algorithm that allowed them to agree on a key in public. This unlocked __public key cryptography__ and, ultimately, secure communications over the internet, like web browsing (implemented in TLS/HTTPS), securing our WiFi (WPA3), or using a VPN (IKE).

## Trapdoor functions

At the core of public key cryptography lies a [trapdoor function](https://en.wikipedia.org/wiki/Trapdoor_function), a mathematical function that's easy to do, but very hard to undo. Alice and Bob each have their own trapdoor function and, by only sharing their respective outputs, can reach the same mathematical result. Meanwhile, Eve will fall right through the trapdoor, taking her eons to figure out what the functions were.
...
But, given that we are doing modulo 60, 32 is not a good choice for $$g$$ either. Out of the 60 outcomes $$\operatorname{mod} 60$$ offers, the powers of 32 modulo 60 occupy only 4: the solution to $$k=1$$ is the same as to $$k=5$$: $$32^{5} \bmod 60 = 32$$. Hence, we quickly shrink the space of possibilities by a factor of 15: only multiples of 4 could produce a remainder of 16. We want the opposite: all options between 0 and $$p$$ should be possible. (I.e., we want $$g$$ to be a __primitive root modulo__ $$\boldsymbol{p}$$.) While there are better choices than 32, [there are no primitive roots modulo 60](https://en.wikipedia.org/wiki/Primitive_root_modulo_n#:~:text=A%20primitive%20root%20exists%20if%20and,odd%20prime%20and%20k%20%3E%200.), and hence we should ditch it altogether. A better choice would be $$g=5$$ and $$p = 7$$, since 5 is a primitive root modulo 7. An even better choice would be to pick a massive, prime $$p$$, and a small $$g$$ that is a primitive root modulo $$p$$.
...
1. Alice and Bob leverage each other's intermediate result and their own private key to compute the same number:
    ...
    Crucially, Eve is clueless about what is this number.
...
__Multiplication__ is the operation that allows us to repeatedly perform addition. Elliptic curve multiplication consists on adding a point over and over. The main problem is defining $$2P = P + P$$ in the absence of another point to compute a line. We get around by taking the tangent of the curve at $$P$$. Then, we simply keep adding $$P$$ to the resulting number to compute higher multiples.
...
__Our trapdoor is elliptic curve point multiplication.__ Given a _base point_ $$P$$ and a factor $$k$$, it is easy to compute $$kP$$. However, given $$kP$$ and $$P$$, it is really hard to guess the value of $$k$$. $$P$$ is the universal base point, $$k$$ is our secret key, and $$kP$$ our public key.
...
where $$p$$ is a prime, defined over $$\mathbb{Z} / p \mathbb{Z}$$. It's hard to see a _curve_ here, although it follows from the continuous case:
...
$$x_{P+Q} = (m^2 - x_P - x_Q) \pmod{p} $$

$$y_{P+Q} = (m (x_P - x_{P+Q}) - y_P) \pmod{p} $$
...
The base point $$P = (9, \sqrt{39\,420\,360} \pmod{2^{255} - 19})$$; the private key is a 256-bit integer $$k$$; and the public key is $$kP$$. Significantly shorter than the 2048-bit one that DH required!
