...
ICA is the paradigmatic solution to the cocktail party problem, a famous problem in signal processing.
...
After preprocessing, we will seek an $$m$$-dimensional vector $$\mathbf{w}$$ that maximizes the non-Gaussianity of the independent component $$\mathbf{s} = X \mathbf{w}$$. We will quantify non-Gaussianity using the excess kurtosis:
...
$$
\hat{K}(\mathbf{s}) = \frac 1 n \sum_{i=1}^n s_i^4 - 3  = \frac 1 n \sum_{i=1}^n (\mathbf{x_{i*}}^\intercal \mathbf{w})^4 - 3.
$$

We will maximize the kurtosis by gradient ascent. Hence, we need the first derivative of the kurtosis:

$$
\frac { d \hat{K}(\mathbf{s}) } { d \mathbf{w} } = \frac 1 n \sum_{i=1}^n 4 (\mathbf{x_{i*}}^\intercal \mathbf{w})^3 \mathbf{x_{i*}} = \frac 4 n \sum_{i=1}^n s_i^3 \mathbf{x_{i*}}.
$$
...
This implementation is a proof of concept, not a production-ready one. The results are clearly not optimal, as we can see by comparing them with the results obtained above.
