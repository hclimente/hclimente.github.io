...
Let's put the above into practice by aiming for the simplest, possibly most inefficient, implementation. We will start with a $$m \times n$$ centered and whitened matrix $$X$$, where $$m$$ is the number of features and $$n$$ the number of samples. We will simply seek an $$n$$-dimensional vector $$\mathbf{w}$$ that maximizes the non-Gaussianity of the independent component $$\mathbf{s} = X \mathbf{w}$$. We will quantify non-Gaussianity using the excess kurtosis:
...
We will maximize the kurtosis by gradient descent. Hence, we need the first derivative of the kurtosis:

$$
\frac { d \hat{K}(\mathbf{s}) } { d \mathbf{w} } = \frac 1 m \sum_{i=1}^m 4 (\mathbf{x_{i*}}^\intercal \mathbf{w})^3 \mathbf{x_{i*}}^\intercal = \frac 4 m \sum_{i=1}^m s_i^3 \mathbf{x_{i*}^\intercal}.
$$
