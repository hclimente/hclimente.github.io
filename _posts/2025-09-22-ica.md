...
where $$a_{ij}$$ is a constant, representing the relative contribution of each component. For instance, it will be small for components sourced far from the microphone. We can arrange the vectors coming from the different microphones into a matrix, and think of it as a matrix factorization problem:

$$
X = AS
$$

where $$A$$ is called the mixing matrix and $$S$$ the independent component matrix. The goal is to estimate both $$A$$ and $$S$$ from $$X$$.
